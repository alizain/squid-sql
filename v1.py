...
if not (major == 3 and minor >= 5):
    ...
    sys.stderr.write(f'Please use Python 3.5+ for this program.\nYou\'re trying to run it with Python {curr_version}.\n')
    sys.exit(1)
...
class Table:
    ...
    def col_config_by_column_ref(self, column_ref):
        ...
        if len(found_columns) < 1:
            raise SquidError(f'column_ref:{column_ref} not found, maybe you haven\'t seleced the required table')
        if len(found_columns) > 1:
            found_table_names = '|'.join(tuple(col_config.table for col_config in found_columns))
            raise SquidError(f'column_ref:{column_ref} is ambigious, found multiple matching columns in tables:{found_table_names}')
    ...
    def apply_where_query(self, where_query):
        if isinstance(where_query, ColumnToLiteralWhere):
            ...
            raise SquidError(f'unknown where query type:{where_query.__class__}')
    ...
    def apply_column_to_column_where_query(self, where_query):
        ...
        if left_col.type != right_col.type:
            raise SquidError(f'cannot compare column:{left_col.name} of type:{left_col.type} with column:{right_col.name} of type:{right_col.type}')
        if where_query.op not in SUPPORTED_OPS_BY_COLUMN_TYPE[left_col.type]:
            raise SquidError(f'non-sensical operator:{where_query.op} for type:{left_col.type}')
    ...
    def apply_column_to_literal_where_query(self, where_query):
        col_config = self.col_config_by_column_ref(where_query.column_term['column'])
        literal_value_key = f'lit_{col_config.type}'
        if literal_value_key not in where_query.literal_term:
            raise SquidError(f'cannot compare column:{col_config.name} of type:{col_config.type} with term:{where_query.literal_term}')
        if where_query.op not in SUPPORTED_OPS_BY_COLUMN_TYPE[col_config.type]:
            raise SquidError(f'non-sensical operator:{where_query.op} for type:{col_config.type}')
        operator_func = OPERATOR_FUNCS[where_query.op]
        ...
        if where_query.column_side == 'left':
            ...
            raise Exception('this should never happen because we\'ve already '
    ...
    def apply_as_exp(self, as_name):
        new_columns = tuple(col_config._replace(table=as_name) for col_config in self.columns)
    ...
    def apply_select_queries(self, raw_select_queries):
        ...
        for select_query in raw_select_queries:
            col_config = self.col_config_by_column_ref(select_query['source']['column'])
            col_name = col_config.name
            if select_query['as']:
                col_name = select_query['as']
            if col_name in found_column_names:
                raise SquidError(f'column as:{col_name} already `select`ed in this query, please use a unique name')
        ...
        new_columns = tuple(col_config._replace(index=index) for index, col_config in enumerate(found_columns))
    ...
    def merge_tables(cls, one, two):
        ...
        two_columns_fixed = tuple(col._replace(index=col.index + to_add) for col in two.columns)
    ...
    def from_file(cls, table_name, raw_rows):
        ...
        columns = cls.validate_and_construct_col_configs(table_name, raw_columns)
        rows = tuple(raw_rows)
    ...
    def validate_and_construct_col_configs(table_name, raw_columns):
        ...
        for index, [col_name, col_type] in enumerate(raw_columns):
            col_config = ColumnConfig(name=col_name, type=col_type, table=table_name, index=index)
            columns.append(col_config)
        return tuple(columns)


class ColumnToLiteralWhere(namedtuple('ColumnToLiteralWhere', [
    'op',
    'left',
    'right',
    'column_side',
    ...
    def __new__(cls, op, left, right):
        if 'column' in left and 'column' in right:
            raise SquidError('this class may be what you want, but it\'s certainly '
            ...
            return super().__new__(cls, op, left, right, column_side='left', literal_side='right')
        elif 'column' not in left and 'column' in 'right':
            return super().__new__(cls, op, left, right, column_side='right', literal_side='left')
        else:
...
class ColumnToColumnWhere(namedtuple('ColumnToColumnWhere', [
    'op',
    'left',
    'right'
])):
...
def run_query(table_loader, raw_query):
    ...
    for from_query in raw_from_queries:
        ...
        if new_table_name in used_table_names:
            raise SquidError(f'table:{new_table_name} has already been declared in this query, try using `as` to give it a distinct name')
...
def pretty_print_table(query_table):
    ...
    max_width = sum(col_max_widths) + ((len(col_max_widths) - 1) * len(col_divider))
    str_row_divider = '-' * max_width
    str_columns = stringify_row(col_divider, col_formatters, columns)
    str_rows = tuple(stringify_row(col_divider, col_formatters, row) for row in all_rows)
    print('\n'.join((str_columns, str_row_divider) + str_rows))

def stringify_row(divider, formatters, row):
    row_stringified = (formatter.format(value) for formatter, value in zip(formatters, row))
    return divider.join(row_stringified)

def create_col_formatters(col_max_widths):
    all_but_last = tuple(f'{{:<{max_width}}}' for max_width in col_max_widths[:-1])
    last = col_max_widths[-1]
    return all_but_last + (f'{{:>{last}}}',)
...
def parse_query_file_and_run_query(filename):
    try:
        with open(filename) as qf:
            ...
            curr_query = json.load(qf)
            run_query(table_loader, curr_query)
    except FileNotFoundError:
        raise SquidError(f'query file:{filename} not found, please make sure it exists')
